    ...
    public static HashMap<Character, BitSet> checkNodesv2(Node node, HashMap<Character, BitSet> hits, BitSet bitSet, int counter) {
        if (node != null) {
            if (node.getRight() != null) {
                ...
                counter++;
                int temp = counter;
                checkNodesv2(node.getRight(), hits, bitSet, counter);
                counter = temp;
            }

            if (node.getLeft() != null) {
                counter++;
                checkNodesv2(node.getLeft(), hits, bitSet, counter);
            }

            if (node.getRight() == null && node.getLeft() == null) {
                if (node.getChararacter().length() == 1) {
                    System.out.println("Char: " + node.getChararacter() + " | Code: " + bitSet.toString());
                    hits.put(node.getChararacter().charAt(0), bitSet);
                    bitSet.clear();
                    counter = 0;
                    return hits;
                } else {
